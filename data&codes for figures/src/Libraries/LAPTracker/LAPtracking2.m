function [ tracks, NO_LINKING_COST, CUTOFFS, deltaD,deltaO,deltaX,deltaY,deltaV,model, displacement] = LAPtracking2( posList, varargin )
...
%Log at bottom
    ...
    prev_pos = [x(slices == min(slices)) y(slices == min(slices)) o(slices == min(slices)) (1:nTracks)' inten(slices == min(slices))];
    ...
    tracks{1} = [prev_pos(:,1:3) ones(cells_in_slice(1),1) prev_pos(:,4) pos(slices == min(slices)) prev_pos(:,5)];       
    ...
    for slice = 1:max(slices) - 1;
        ...
        next_pos = [x(slices == slice + 1) y(slices == slice + 1) o(slices == slice+1) inten(slices == slice + 1)];
        ...
        %Update the states
        positions =  zeros(cells_in_slice(slice + 1),6);
        end_cost = 0;
		...
		for r = 1:cells_in_slice(slice + 1)
			if(p12(r) <= cells_in_slice(slice))
                                %    x,y                i
                positions(r,:) = [next_pos(r,1:3), prev_pos(p12(r),4), pos_i(r), next_pos(r,4)];
        ...
        %                   [x y o frame id]
        tracks{slice + 1} = [positions(:,1:3) repmat(slice + 1,cells_in_slice(slice + 1),1) positions(:,4:5) positions(:,6)];
