function [Tab ] = CompileVariables2( AllData )
    ...
    for j = 1:length(AllData)
        ...
        %             
            ...
            %                 w = waitforbuttonpress;
                ...
                T = [T; table(... 
                      runs.distance(1:end-2), ... %1
                      runs.distance(2:end-1), ...
                      runs.distance(3:end), ...
                      runs.length(1:end-2), ...
                      runs.length(2:end-1), ...%5
                      runs.length(3:end), ...
                      runs.speed(1:end-2), ...     
                      runs.speed(2:end-1), ...  
                      runs.speed(3:end), ...  
                      runs.start.density(1:end-2), ...
                      runs.start.density(2:end-1), ...
                      runs.start.density(3:end), ...
                      runs.stop.density(2:end-1), ...
                      (runs.stop.density(1:end-2) - runs.start.density(1:end-2)) ./ runs.length(1:end-2), ... %10
                      (runs.stop.density(2:end-1) - runs.start.density(2:end-1)) ./ runs.length(2:end-1), ...
                      (runs.stop.density(3:end) - runs.start.density(3:end)) ./ runs.length(3:end), ...
                      runs.start.frame(1:end-2), ...
                      runs.start.frame(2:end-1), ...
                      runs.start.frame(3:end), ...
                      runs.start_dist_to_agg_norm(1:end-2), ...
                      runs.start_dist_to_agg_norm(2:end-1), ... %15
                      runs.start_dist_to_agg_norm(3:end), ... %15
                      runs.phi(1:end-2), ...
                      runs.phi(2:end-1), ...
                      runs.phi(3:end), ...
                      runs.beta(1:end-2), ...
                      ...
                      runs.theta(2:end-1), ...
                      runs.theta(3:end), ...
                      runs.start.x(1:end-2), ...
                      runs.start.x(2:end-1), ...
                      runs.start.x(3:end), ...
                      runs.start.y(1:end-2), ...
                      runs.start.y(2:end-1), ...
                      runs.start.y(3:end), ...
                      runs.stop.x(1:end-2), ...
                      runs.stop.x(2:end-1), ...
                      runs.stop.x(3:end), ...
                      ...
                      runs.stop.y(2:end-1), ...
                      runs.stop.y(3:end), ...
                      runs.orientation(1:end-2), ...
                      runs.orientation(2:end-1), ...
                      runs.orientation(3:end), ...
                      runs.state(1:end-2), ...
                      runs.state(2:end-1), ...
                      runs.state(3:end), ...
                      runs.id(1:end-2)) AggProps(1:end-2,:)];
            else
                runs = subStruct(runs,1:(length(runs.start.x)-1),'r');
                runs.theta = theta;
                if(any(imag(theta)))
        ...
        T(T{:,17} < StartFrame,:) = [];
        T(T{:,18} > StopFrame,:) = [];
        T{:,17} = T{:,17} - StartFrame;
        T{:,18} = T{:,18} - StartFrame;
    ...
    if(HAS_AGG_DATA)
        Tab.Properties.VariableNames = [{
                            ...
                            'Rt0','Rt1', 'Rt2',... %3
                            'Rs0','Rs1', 'Rs2', ... %5
                            'rho0','rho1','rho2','rho3', ... %7
                            'drho0','drho1', 'drho2',... %10
                            'TSS0','TSS1','TSS2', ... %12
                                    'Dn1' ,'Dn2' , 'Dn3' , ... %14
                                    'phi0','phi1','phi2', ... %16
                                    'beta1','beta2', 'beta3',... %18
                              'theta1','theta2','theta3', ...
                            'xstart0','xstart1', 'xstart2',...
                            'ystart0','ystart1','ystart2', ...
                            'xstop0','xstop1', 'xstop2',...
                            'ystop0','ystop1','ystop2', ...
                            'orientation0', ...
                            'orientation1' ,...
                            'orientation2' ,...
                            'state0', ...
    ...
    for curMovie = 1:length(unique(Tab.set))
        ...
        for j = 1:length(result)
            ...
            surrounding = T.orientation1(idx(2:end-1));
