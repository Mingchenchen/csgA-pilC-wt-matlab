...
tiff_file = DIFA_RAW_RIPPLE_MOVIES('RIPPLE_6');

processed_folder = [PROJECT_BASENAME, '/data/processed/', DIFA_RIPPLE_FOLDERS('RIPPLE_6')];
check_folder = [PROJECT_BASENAME '/reports/DataChecks/', DIFA_RIPPLE_FOLDERS('RIPPLE_6')];
...
if(~HEADLESS)
    figure
    detectCellsRegionProps(tiff_file,'MinLevel',5.1,'check',1,'small_cutoff',3);
end

%%
posList1 = detectCellsRegionProps(tiff_file,'MinLevel',5.1,'small_cutoff',3);
...
%%
% load(density_file);
% load([save_folder 'tracks'])
% 
% m_tracks = createMTracks(tracks,Kripple); 
% 
% if(SAVE_RESULTS)
%     if(~OVERWRITE && exist([save_folder 'm_tracks.mat'],'file') > 0)
%         error('File m_tracs.mat already exists, set OVERWRITE = true to overwrite');
%     end
%     save([save_folder 'm_tracks'],'m_tracks')
% end
