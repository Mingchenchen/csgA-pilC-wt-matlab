...
tiff_file = CSGA_IN_CSGA_RAW_DEV_MOVIES('Exp1_34');

processed_folder = [PROJECT_BASENAME, '/data/processed/', CSGA_IN_CSGA_DEV_FOLDERS('Exp1_34')];
check_folder = [PROJECT_BASENAME '/reports/DataChecks/', CSGA_IN_CSGA_DEV_FOLDERS('Exp1_34')];

mkdir(check_folder)
mkdir(processed_folder)

%%
screePlot(tiff_file,4:0.25:7);
...
if(~HEADLESS)
    figure
    detectCellsRegionProps(tiff_file,'MinLevel',6.5,'check',1,'small_cutoff',3);
end

%%
posList1 = detectCellsRegionProps(tiff_file,'MinLevel',6.5,'small_cutoff',3);
...
m_tracks = createMTracks(tracks,nan(2^10,2^10,max(tracks.frame))); 
