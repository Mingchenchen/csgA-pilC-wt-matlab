...
tiff_file = CSGA_IN_CSGA_RAW_DEV_MOVIES('Exp1_32');

processed_folder = [PROJECT_BASENAME, '/data/processed/', CSGA_IN_CSGA_DEV_FOLDERS('Exp1_32')];
check_folder = [PROJECT_BASENAME '/reports/DataChecks/', CSGA_IN_CSGA_DEV_FOLDERS('Exp1_32')];

mkdir(check_folder)
mkdir(processed_folder)

%%
screePlot(tiff_file,5:0.5:12);
...
if(~HEADLESS)
    figure
    detectCellsRegionProps(tiff_file,'MinLevel',9.5,'check',1,'small_cutoff',3);
end

%%
posList1 = detectCellsRegionProps(tiff_file,'MinLevel',9.5,'small_cutoff',3);
