function [R,xi] = bootstrap_runs_in_beta2(xi,T,WINDOW,NBOOT)
    ...
    tic
        [R.towards.Rd.ci,R.towards.Rd.samples] = bootci(NBOOT,{bootfun,T.Dn1,T.Rd1},'type','cper','Weights',weights(:)/sum(weights(:)));
    toc
       % [R.away.Rd.ci,R.away.Rd.samples]       = bootci(NBOOT,{bootfun,T.Dn1(cos(T.beta1) < -CUTOFF),T.Rd1(cos(T.beta1) < -CUTOFF)},'type','cper','Weights',weights(cos(T.beta1) < -CUTOFF)/sum(weights(cos(T.beta1) < -CUTOFF)));
        ...
        [R.towards.Rs.ci,R.towards.Rs.samples] = bootci(NBOOT,{bootfun,T.Dn1,T.Rs1},'type','cper','Weights',weights(:)/sum(weights(:)));
       % [R.away.Rs.ci,R.away.Rs.samples]       = bootci(NBOOT,{bootfun,T.Dn1(cos(T.beta1) < -CUTOFF),T.Rs1(cos(T.beta1) < -CUTOFF)},'type','cper','Weights',weights(cos(T.beta1) < -CUTOFF)/sum(weights(cos(T.beta1) < -CUTOFF)));
        ...
        [R.towards.Rt.ci,R.towards.Rt.samples] = bootci(NBOOT,{bootfun,T.Dn1,T.Rt1},'type','cper','Weights',weights(:)/sum(weights(:)));
