classdef ClosedLoopProbs3<Probabilities
	%
    ...
    methods
        function obj = ClosedLoopProbs3(Mdlb_goin,Mdlb_goout,Mdlr,Mdlst,Init,InitFrames)
	        %
	        % Constructor
	        %
	        %params:
            ...
            obj@Probabilities(Mdlb_goin,Mdlr,Mdlst);
            obj.Init = Init;
            obj.InitFrames = InitFrames;
            obj.Mdlb_goout=Mdlb_goout;
            obj.Mdlb_goin=Mdlb_goin;
        end
        ...
        function [time,spd,theta1,beta1,dist,state1] = getTransitions(obj,TSS1,phi0,chi,Dn1,rho1,state0)   
            ...
            %Get new beahvior
            if cos(beta1)>=0
            [neighbors, dist(3)] = obj.Mdlb_goin(s).KNN.sample([TSS1, ...
                                               NaN, ...rho1
                                              NaN,... cos(beta1), ...
                                         NaN]);%  Dn1]);
            time = obj.Mdlb_goin(s).T.Rt1(neighbors) * 2;
            spd =  obj.Mdlb_goin(s).T.Rs1(neighbors) / 2;                       
            else 
                [neighbors, dist(3)] = obj.Mdlb_goout(s).KNN.sample([TSS1, ...
                                               NaN, ...rho1
                                               NaN,...cos(beta1), ...
                                             NaN]);%  Dn1]);
                time = obj.Mdlb_goout(s).T.Rt1(neighbors) * 2;
                spd =  obj.Mdlb_goout(s).T.Rs1(neighbors) / 2;                         
            end
