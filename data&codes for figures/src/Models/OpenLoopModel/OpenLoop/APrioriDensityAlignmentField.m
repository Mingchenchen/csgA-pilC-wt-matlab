classdef APrioriDensityAlignmentField<Field
    ...
    properties
        ...
        loadpre;%whether load previous run;
        BINS; % size(density,1);
    ...
    methods
        ...
        function obj = APrioriDensityAlignmentField(density,start_density,agg_tracks,nCells,loadpre) 
...
%load cells at excat positions as experiment
%             load('tracks1');
%             tracks=subStruct(tracks1,tracks1.frame==1);
%             for i=1:length(tracks.frame)
%                 x=tracks.x(i)/1920*986;y=tracks.y(i)/1440*740;
%                 obj.cellList{i} = MyxoCell(obj,modelStats,probs,i,x,y);
%             end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            else
                for i=1:length(obj.loadpre{2})
                    x=obj.loadpre{2}(i);y=obj.loadpre{3}(i);
                    obj.cellList{i} = MyxoCell(obj,modelStats,probs,i,x,y);
                end
            end
            obj.step(1);
