...
SAVE = true;
SAVE_FOLDER = [PROJECT_BASENAME '/reports/figures/simulation/development/csgASims/'];
if(SAVE)
    mkdir(SAVE_FOLDER)
end
   
%% Full CsgA Simulation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SIM_NAME = 'PUB_NEW_2';
...
figure, clf, hold on;
    ...
    boundedline(xi,sim_frac_mean,[sim_frac_std,sim_frac_std],'-','cmap',[0 1 0],'alpha')
    
    xi = (1:length(ucsga)) / 2 / 60;
    %boundedline(xi,uwt,[swt,swt],'-','cmap',[0 0 0],'alpha') %Compare with WT
    boundedline(xi,ucsga,[scsga,scsga],'-','cmap',[1 0 0],'alpha') %Compare with CsgA
   
%% CsgA no bias in non-persisetnt state
SIM_NAME = 'PUB_NEW_3';
...
sim_frac_std = std(simFracCurves(prerun_length:end,:),[],2) .* 100;
xi = (1:length(sim_frac_mean)) / 2 / 60
boundedline(xi,sim_frac_mean,[sim_frac_std,sim_frac_std],'-','cmap',[0 0 1],'alpha')
...
sim_frac_std = std(simFracCurves(prerun_length:end,:),[],2) .* 100;
xi = (1:length(sim_frac_mean)) / 2 / 60
boundedline(xi,sim_frac_mean,[sim_frac_std,sim_frac_std],'-','cmap',[0 1 1],'alpha')
